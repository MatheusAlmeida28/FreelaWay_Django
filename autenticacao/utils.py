...
def password_is_valid(request, password, confirm_password):
    if len(password.strip()) == 0:
        messages.add_message(request, constants.WARNING, 'senha em branco ou confirmar senha ficou em branco')
        return False
    ...
    if len(password) < 6:
        ...
        messages.add_message(request, constants.ERROR, 'As senhas não coincidem! Verifique se não digitou errado')
        return False
